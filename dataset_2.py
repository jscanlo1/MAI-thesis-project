...
class CustomDataset(Dataset):
    def __init__(self, text, deepMoji_input ,truth_labels,  transform=None, target_transform=None):
        self.text = text
    ...
    def __getitem__(self, idx):
        ...
        return torch.DoubleTensor(text_item), torch.DoubleTensor(emo_item) , torch.LongTensor(label)
    ...
    def processing_data(path, dataset_type_type):
        ...
        if dataset_type == 'AAAI':
            ...
            if dataset_type_type == 'train':
                ...
                fake_news_features_input = torch.load("fake_news_features/AAAI/FN_AAAI_train.pt")
            elif dataset_type_type == 'val':
                deepMoji_inputs = torch.load("deepMoji_inputs/AAAI/AAAI_val.pt")
                fake_news_features_input = torch.load("fake_news_features/AAAI/FN_AAAI_val.pt")
            elif dataset_type_type == 'test':
                deepMoji_inputs = torch.load("deepMoji_inputs/AAAI/AAAI_test.pt")
                fake_news_features_input = torch.load("fake_news_features/AAAI/FN_AAAI_test.pt")
        ...
        return CustomDataset(fake_news_features_input, deepMoji_inputs, truth_label_input)
